...
The script stops before target classification and produces only the formal
Diagnostic26 source feature set used by Question 2.
"""
...
def ablation_sets(all_names,selected):
    ...
    return {"A_all_candidate":all_names,"B_diagnostic26":current,"C_time_only":time_names,"D_time_frequency":time_names+frequency,"E_time_frequency_envelope":time_names+frequency+envelope,"F_time_frequency_envelope_order":time_names+frequency+envelope+order,"B_without_acf_first_peak_s":[name for name in current if name!="acf_first_peak_s"],"B_without_order_dominant":[name for name in current if name!="order_dominant"]}
...
def window_sensitivity(source,meta,default_raw,out):
    ...
    for window in (8192,16384,32768):
        start=time.perf_counter();raw=pd.DataFrame(rows_for(source,FS_MAIN,window,window//2));selected,quality=screen_features(raw);diagnostic=[name for name in selected if name not in SUSPECT_FEATURES];features=aggregate(raw,meta,diagnostic);results,_=lolo(features,diagnostic,"diagnostic_default");runtime=time.perf_counter()-start
    ...
    text="# 窗口长度敏感性\n\n"+decision+"\n\n```csv\n"+result.to_csv(index=False,float_format="%.4f",lineterminator="\n")+"```\n\n所有结果均按原始 MAT 文件做 LOLO；本表不是目标域准确率。特征配置固定为 Diagnostic26，未在测试载荷上进行监督选择。\n";(out/"validation"/"window_selection_summary.md").write_text(text,encoding="utf-8");return result
def export_diagnostic(raw,selected,out):
    diagnostic=[name for name in selected if name not in SUSPECT_FEATURES];base=list(ID_COLUMNS);save_df(out/"features_source_diagnostic.csv",raw[base+diagnostic]);(out/"feature_names_diagnostic.json").write_text(json.dumps({"features":diagnostic,"n_features":len(diagnostic),"purpose":"Question 2 source-domain diagnosis","excluded_suspects":list(SUSPECT_FEATURES)},ensure_ascii=False,indent=2),encoding="utf-8");return diagnostic
def main():
